%% ===================================================================
%% QSS 20 final paper. PNAS single-column mathematics template.
%%
%% Build: pdflatex final_paper.tex (twice, for cross-references).
%% The PNAS class and its support files sit beside this file, so the paper
%% compiles from a fresh clone; figures and tables are read from ../output/,
%% which the notebooks generate.
%%
%% On Overleaf: upload the output/ figures and tables and set \OUT to {.}
%% The DRAFT watermark is already off (pnas-new.cls, displaywatermark=false).
%% ===================================================================
...
%% Generated figures and tables live in output/. On Overleaf, upload them and
%% set \OUT to {.}
...
\title{Which Rural Places Move Kids Up? Neighborhood Correlates of Upward
Mobility Within Rural Labor Markets}

\author[a]{Maxwell Fortner}
\affil[a]{Department of Quantitative Social Science, Dartmouth College, Hanover, NH 03755}

\leadauthor{Fortner}

\significancestatement{Research on how neighborhoods shape children's adult
earnings is built almost entirely on cities, and the Opportunity Atlas notes
that growing up rural helps children in some regions and hurts them in others
without saying what separates the two. Comparing rural Census tracts against
other tracts in the same local labor market, the share of single-parent
households is the largest measured correlate of adult income rank for children
raised poor, while local economic conditions show no measurable association.
The effect of region, however, dwarfs that of any tract characteristic.}

\authorcontributions{M.F. designed the analysis, wrote the code, and wrote the paper.}
\authordeclaration{The author declares no competing interests.}

\keywords{intergenerational mobility $|$ rural America $|$ neighborhood effects $|$ Opportunity Atlas}

\begin{abstract}
Research on neighborhoods and intergenerational mobility is
...
\dates{QSS 20 final project. Compiled on \today}
...
\noindent\textbf{Neighborhood effects on mobility.} Chetty and Hendren estimate that
...
\noindent\textbf{Correlates of mobility across places.} A parallel descriptive
...
\noindent\textbf{Mechanisms: schools, sorting, and race.} Two of my predictors are
...
\noindent\textbf{Source and time window.} All data come from the Opportunity
...
\noindent\textbf{Unit of analysis.} In the original source each row is a 2010 Census
...
\noindent\textbf{Outcome.} \texttt{kfr\_pooled\_pooled\_p25}: the mean percentile
...
\noindent\textbf{Predictors.} Ten tract characteristics, fixed before estimation to
...
\noindent\textbf{Rural definition and analysis sample.} Rural means fewer than 100
...
\noindent\textbf{Missing cells.} Counts of tracts with no value, per field, are in
\texttt{output/table2\_missing\_cells.tex} in the repository.
...
\noindent\textbf{Limitations of the source.} (i) The Atlas reports estimates, not
...
after the focal cohorts' childhood years. (iii) Tracts use 2010 boundaries but
the children grew up in the 1980s and 1990s. (iv) Complete-case estimation
drops 668 of 17{,}978 rural tracts (3.7\%).
...
estimates and writes every figure and table here. All paths resolve relative to
\texttt{code/utils.py}.

\noindent\textbf{Cleaning and merging.} The two Atlas tables are joined on the tract
key \texttt{(state, county, tract)} with an \textbf{inner join}, validated as
\textbf{one-to-one} (\texttt{pandas.merge(..., how="inner",
validate="one\_to\_one")}), so a silent many-to-many blowup would raise rather than inflate the sample. Row counts are printed
before and after: 73{,}278 outcome rows and 74{,}044 covariate rows in,
73{,}199 matched rows out. I then drop 1{,}241 tracts missing either the
mobility outcome or population density, leaving \textbf{71{,}958} tracts. This
is an exact-match merge on a shared numeric key; no fuzzy matching or blocking
is involved.

\noindent\textbf{Derived variables.} The density cutoffs are defined once in
...
\noindent\textbf{Estimation.} For the rural sample I estimate
\begin{equation}
...
\noindent\textbf{Explained variance.} I report the \emph{incremental} $R^2$: the
...
\noindent\textbf{Precision screen.} Atlas estimates resting on few children are
...
\noindent\textbf{Rural versus dense-urban comparison.} To ask whether the rural
...
\noindent\textbf{What this design does not identify.} This is descriptive, not
...
\noindent\textbf{Family structure is the largest adjusted correlate in rural tracts.}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.80\linewidth]{fig2_rural_ols_coefficients.png}
...
\noindent\textbf{Region explains far more than tract characteristics do.}
...
Most of what predicts a rural child's adult income
rank is which regional labor market they grew up in, not which tract inside it.

\noindent\textbf{The result survives a precision screen.} Refitting on the 14{,}741 rural tracts whose estimates rest on at least 200
children (\texttt{output/table4\_count\_sensitivity.tex}) leaves the ranking
intact. Single-parent share is essentially
...
of noise around zero, not unstable associations; that table marks significance so this is
visible without the surrounding text.

\noindent\textbf{The rural pattern is not a scaled-down urban one.}
...
not be read as poverty benefiting children: poverty share is heavily collinear
with the other predictors ($r=0.64$ with mean household income, $0.62$ with
single-parent share), so a coefficient that reverses its bivariate sign here is
best understood as suppression. The comparison is \textbf{exploratory}: the
dense-urban model has only 28 commuting-zone clusters, below the range where
cluster-robust standard errors are reliable~\cite{cameron2015practitioner}.
It compares shape, not magnitude.

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.80\linewidth]{fig3_rural_dense_coefficients.png}
...
travel with children. The predictors are strongly collinear (mean income with
BA share, $r=0.78$), so the poverty coefficient is a suppression artifact.
``Rural'' is my own density cutoff, not the Census Bureau's. And the
dense-urban comparison rests on 28 clusters.
