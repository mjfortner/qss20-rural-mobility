...
%% Draft-only marker for text Max still needs to write. Set the second
%% argument of \newcommand to {} to make these disappear before submitting.
\newcommand{\towrite}[1]{\medskip\noindent\textcolor{red}{\textbf{[TO WRITE --- #1]}}\medskip}
...
children in some regions and hurts them in others without identifying what
separates the two. I use Opportunity Atlas tract tables to ask which
neighborhood characteristics distinguish rural Census tracts with
above-average upward mobility from otherwise similar rural tracts. Comparing
...
standard deviation of rural tract mobility. Local economic conditions --- job
growth, employment, commute time, and mean household income --- are
statistically indistinguishable from zero once region is held fixed. Ten tract
...
policy is usually designed around urban mechanisms --- housing vouchers, school
choice, transit access --- that may not describe what varies across rural
places.

This paper asks a deliberately descriptive question: \textbf{among rural Census
...
Family structure dominates. Among rural tracts in the same commuting zone,
single-parent household share carries the largest adjusted association with
children's adult income rank --- larger than mean household income, poverty,
employment, job growth, commute time, or third-grade math scores --- while the
local economic indicators are indistinguishable from zero. This extends the
...
\towrite{2--4 sentences of your own motivation, at the end of this section or
folded into the first paragraph. See the notes file: you already wrote a
version of this in the Me Search assignment and Milestone 1.}
...
ages~\cite{chetty2017exposure,chetty2018county}. The Moving to Opportunity
...
\emph{which} observable features of a place track opportunity, and it is
estimated almost entirely on urban movers.
...
comparing tracts only within commuting zones.

\paragraph{Mechanisms: schools, sorting, and race.} Achievement gaps by family
income have widened across cohorts~\cite{reardon2011}, income segregation has
risen~\cite{reardonbischoff2011}, school quality tracks measured achievement
gaps~\cite{hanushek2006}, and mobility differs sharply by race in ways
neighborhood characteristics alone do not explain~\cite{chetty2020race}. This
last point shapes how I read my own estimates: the large positive coefficient
on share white is a conditional association on cross-sectional data, consistent
with documented national race gaps, and is not interpretable as a place effect
under this design.

\towrite{one or two sentences positioning your work against a rural-specific
citation. See the notes file --- this is the one real gap in the related-work
section and the rubric explicitly asks you not to present your analysis as
...
the two files). No API key is required and the raw files are not committed; the
pull notebook reproduces them. Outcomes describe the 1978--1983 birth cohorts measured in
...
parents sat at the 25th percentile, pooling across child race and gender: a
value of 0.43 means those children reached the 43rd percentile of the adult
income distribution on average. Because the outcome is a rank, coefficients
read as percentile points once multiplied by 100.
...
imperfect proxy --- a low-density tract on the edge of a metropolitan area is
economically suburban --- though the commuting-zone fixed effects in
...
tract's estimate.}
\label{tab:sample}
\input{\OUT/table1_analysis_sample.tex}
\end{table}
...
for 1.6\% of tracts --- Opportunity Insights suppresses estimates resting on
too few children --- and at the same 1.6\% rate among rural tracts. Job growth
...
\label{tab:missing}
\footnotesize
\input{\OUT/table2_missing_cells.tex}
\end{table}
...
\paragraph{Derived variables.} Density is binned into five categories (rural
$<$100; small town 100--500; suburban 500--2k; urban 2k--10k; dense urban
$\geq$10k people per square mile) with edges defined once in
\texttt{utils.DENSITY\_BINS}, and the child count
(\texttt{pooled\_pooled\_count}) is carried through the merge for the screen.
...
deviation means something different in each group: the standard deviation of
...
causal. There is no exogenous variation in tract characteristics and no source
of causal identification: commuting-zone fixed effects remove between-region
composition, but not the sorting of families into tracts within a region, and
not unobserved tract differences. Coefficients are conditional associations,
and I use ``associated with'' throughout deliberately.
...
\paragraph{Mobility varies more within density categories than between them.}
Figure~\ref{fig:density} plots tract mobility across the five density
categories. Median mobility is modestly higher in rural tracts than dense urban
ones, but the interquartile range within every category is wider than the gap
between category medians. Rurality alone is not a useful predictor of a child's
outcome; the variation worth explaining is \emph{among} rural places.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.72\linewidth]{fig1_mobility_by_density.png}
\caption{\textbf{Within-category mobility variation exceeds median
differences.} Distribution of mean adult household income rank for children of
25th-percentile parents, by 2000 population density category, across 71{,}958
tracts. Boxes show the interquartile range, whiskers the 5th--95th percentiles,
and the dashed line the national tract median. The spread inside each category
is larger than the differences between category medians.}
\label{fig:density}
\end{figure}
...
percentile points, so this is roughly a quarter of a standard deviation --- the
largest single association in the model, and about a third larger in magnitude
than the next-largest coefficient.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.78\linewidth]{fig2_rural_ols_coefficients.png}
...
\label{tab:ols}
\input{\OUT/table3_ols_results.tex}
\end{table}
...
employment share --- but all three are null in the main model, so these are
movements of noise around zero, not unstable associations; the table marks
significance so this is visible without the surrounding text.
...
\label{tab:screen}
\footnotesize
\input{\OUT/table4_count_sensitivity.tex}
\end{table}
...
narrow. It is reported to compare the \emph{shape} of the two patterns, not to
support inference about dense-urban magnitudes.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.78\linewidth]{fig3_rural_dense_coefficients.png}
...
within-labor-market comparison, and it survives a precision screen. The second
...
another and the poverty coefficient in particular is a suppression artifact.
``Rural'' is a density cutoff of my own choosing rather than the Census
Bureau's definition. And the dense-urban comparison rests on 28 clusters and is
exploratory.

\towrite{your own closing: what you would do next, and what you would want a
reader to take away. See the notes file for concrete candidates.}
...
%% ====================================================================
%% This appendix is excluded from the 6-page limit.
%% Everything below is a scaffold. The episodes are real and are drawn from
%% the project log, but the transcripts and the judgments have to be yours.

\towrite{this entire appendix --- see the notes file. The structure below is a
scaffold with the real episodes filled in; you need to paste the transcripts
and write the accept/reject judgments in your own voice.}

\subsection*{Workflow}

I used an AI coding assistant throughout the analysis and to build the project
website. The assistant wrote most of the notebook scaffolding and figure code;
I specified the research design, chose the specification, and reviewed every
numeric claim against the printed diagnostics.

\subsection*{What I accepted}

\towrite{2--3 concrete examples. Candidates from the log: the commuting-zone
fixed-effects specification; the dot-and-whisker coefficient plot that replaced
the standardized mean-difference chart after Milestone 1 feedback; the
one-to-one merge validation and before/after row-count diagnostics.}

\subsection*{What I rejected or corrected}

A review pass over the Milestone 2 figures surfaced six problems, all of which
were then fixed:

\begin{enumerate}\itemsep2pt
\item \textbf{Group-specific standardization inflated the rural--urban
comparison.} Rural and dense-urban models were each standardized within their
own sample, making the dense-urban single-parent coefficient look 3.3 times
larger than the rural one when the underlying ratio is 1.47. Fixed by
standardizing both on pooled standard deviations.
\item \textbf{$R^2=0.661$ was misattributed to the covariates.} The
commuting-zone indicators alone account for 0.564; the covariates add 0.097.
The paper now reports the incremental value.
\item \textbf{A takeaway cell was left as a TODO} and would have shipped as
placeholder text.
\item \textbf{Inference on 28 clusters} was presented without caveat.
\item \textbf{Sign flips under the count screen were framed as instability}
when all three affected coefficients were null in the main model.
\item \textbf{The positive poverty coefficient traveled without its
caveat} into figure text, where a reader could take it as evidence that poverty
benefits children.
\end{enumerate}

\subsection*{Where the assistant went wrong}

The clearest failure was not a coding error. Asked about missingness, the
assistant predicted that suppression would fall disproportionately on sparse
rural tracts and \emph{wrote figure prose asserting that pattern before the
numbers were computed}. The data show the opposite: outcome suppression runs at
1.6\% in rural and non-rural tracts alike (Table~\ref{tab:missing}). The
prediction was plausible, stated confidently, and wrong, and it had already
been written into a draft of the Milestone 1 memo before it was checked.

\towrite{your own reflection here. What this taught you about where to place
your own verification, and any other episode you remember where the assistant
was confidently wrong.}

\subsection*{Transcripts}

\towrite{paste the transcripts, or link them. Check whether the assignment
wants them inline or as a link.}
